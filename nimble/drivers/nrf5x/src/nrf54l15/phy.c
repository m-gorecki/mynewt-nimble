...
phy_ppi_init(void)
{
    ...
    NRF_RADIO->PUBLISH_BCMATCH = DPPI_CH_PUB(RADIO_EVENTS_BCMATCH_RADIO);
    ...
    NRF_PPIB10->SUBSCRIBE_SEND[1] = DPPI_CH_PUB(RADIO_EVENTS_BCMATCH_RADIO);
    NRF_PPIB00->PUBLISH_RECEIVE[1] = DPPI_CH_PUB(RADIO_EVENTS_BCMATCH_AAR);

    /* Enable channels we publish on */
    NRF_DPPIC10->CHENSET = DPPI_CH_ENABLE_ALL;
    NRF_DPPIC00->CHENSET = (DPPIC_CHEN_CH0_Msk | DPPIC_CHEN_CH1_Msk);
